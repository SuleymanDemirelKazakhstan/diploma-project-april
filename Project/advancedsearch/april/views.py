...
def getbibtext(pub_list):
    ...
    for item in pub_list:
        ...
        wherePublished = wherePublished.replace('&', '\&')
        ob = ob + wherePublished
        if str(item['PP']) != "":
...
def exporttopdf(request):
    ...
    return render_to_pdf(request, template_name, context, filename=f"{AuthorInf['surname']}_{AuthorInf['name']}.pdf")
