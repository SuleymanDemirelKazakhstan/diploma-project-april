...
def getAuthors(request):
    if request.method == 'POST':
        ...
        print('Evaluation time for one getAuthors request: ' )
        print(b - a)

        print('Number of sources: ' + str(coun))
...
def getscopusauthor(request):
    if request.method == 'POST':
        ...
        resultp = [None]
        print(type(publicationsList[0]))
        print(type([]))
        if(len(publicationsList[1])> 0):
...
def getPublications(request):
    ...
    if final_authors[int(index)].iexpId != "":
        ...
        final_authors[int(index)].iexpId = str(final_authors[int(index)].iexpId)
    r3 = [None]
    scholar_author(final_authors[int(index)].surname, final_authors[int(index)].name, r3, 0)
    if r3[0] !=[]:
        final_authors[int(index)].scholarUrl = r3[0][0].scholarUrl
    ...
    result = [author, resultp]
    b = datetime.datetime.now().replace(microsecond=0)
    print('Evaluation time for one getPublications request: ')
    print(b - a)
    print('Count of publications: ' + str(len(resultp)))
    return Response(result, content_type='application/json')
